...
/*
  ...
  async img_clssfctn_msg_handler() {
    let imgSize;
  ...
  handleExistance(params, src, id) {
    ...
    if (params.includes(src)) {
      ...
      if (params.length > 0) {
        this.getCurrClckdImg("clssfctn_slctd_img", params[params.length - 1].substring(src.lastIndexOf("/") + 1, src.length));
      }
      else {
      ...
      this.highliter(id);
      this.getCurrClckdImg("clssfctn_slctd_img", src.substring(src.lastIndexOf("/") + 1, src.length));
    }
  }
  highliter(imgId) {
  ...
  next() {
    ...
    if (this.batnum < this.getBatchNumber() - 1) {
      ...
      Shiny.onInputChange("img_clssfctn_ud_btch_tckr", (this.batnum + 1) + " / " + this.getBatchNumber());
      this.clearImages();
      this.displayImages(this.batnum);
    }
    else {
      Shiny.onInputChange("img_clssfctn_ud_btch_tckr", this.getBatchNumber() + " / " + this.getBatchNumber());
      this.clearImages();
